...
/*const yearFormat ="%Y";*/
const yearData = dataset.map(function (d) {
  var parsedTime = d.Time.split(':');
...
//For X-Axis
const maxDate = d3.max(dataset, d => d['Year']);
const minDate = d3.min(dataset, d => d['Year']);


//Format Time
const specifier = "%M:%S";
/*const parsedData = dataset.map(function(d) {
  return d3.timeParse(specifier)(d['Time'])
})*/
//console.log(JSON.stringify(parsedData));
//For Y-Axis
//const paddingTime = d3.timeParse(specifier)("00:04")
const maxTime = d3.max(dataset, d => d['Time']);
const minTime = d3.min(dataset, d => d['Time']);

console.log(JSON.stringify(maxTime));
//Create xScale
...
scale(yScale).
ticks(12).
tickFormat(function (d) {return d3.timeFormat(specifier)(d);});

//Append ground and insert axes
...
//Formatting integer -> http://bl.ocks.org/zanarmstrong/05c1e95bf7aa16c4768e
//Add Second -> https://www.tutorialspoint.com/How-to-add-10-seconds-to-a-JavaScript-date-object
...
.on("mouseover", function (d) {
  ...
  style("opacity", .9);
  //ADD Text
  div.html("<p>" + d["Name"] + ": " + d["Nationality"] + "</br>" + "Year: " + d["Year"] + "," + "Time: " + d["Time"] + "</p>" + "<p>" + d["Doping"] + "</p>").
